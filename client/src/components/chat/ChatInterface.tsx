...
export default function ChatInterface() {
  const { activeModel, conversations, loading, apiConnected, sendChatMessage } = useChat();
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const currentConversation = conversations[activeModel] || [];
  ...
  // Handle example message selection from welcome screen
                ...
                <div className="flex-1">
                  ...
                  <div className={`bg-${modelConfig.color}-50 p-3 rounded-lg shadow-sm border border-${modelConfig.color}-100`}>
                    <div className="flex items-center">
                      <div className={`w-2 h-2 bg-${modelConfig.color}-400 rounded-full mr-1 animate-pulse`}></div>
                      <div className={`w-2 h-2 bg-${modelConfig.color}-400 rounded-full mr-1 animate-pulse delay-100`}></div>
                      <div className={`w-2 h-2 bg-${modelConfig.color}-400 rounded-full animate-pulse delay-200`}></div>
                    </div>
                  </div>
                </div>
              </div>
