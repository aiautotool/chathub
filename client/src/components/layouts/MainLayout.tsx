      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between p-4 border-b border-neutral-200 bg-white">
          ...
          <div className="flex items-center">
            <div 
              className={`flex items-center justify-center w-6 h-6 rounded bg-${modelConfig.color}-500 text-white mr-2`}
            >
              <IconForModel name={modelConfig.icon} />
            </div>
            <h1 className="font-medium">{modelConfig.name}</h1>
          </div>
...
interface IconForModelProps {
  name: string;
}

function IconForModel({ name }: IconForModelProps) {
  switch (name) {
    case 'brain':
      return (
